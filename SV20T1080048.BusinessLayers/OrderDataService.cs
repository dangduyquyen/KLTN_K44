...
namespace SV20T1080048.BusinessLayers
{
    public class OrderDataService
    {
        ...
        {
            return orderDB.List(1, 0, searchValue, 0,0).ToList();
        }
        public static List<Order> ListOrders(int customerID = 0)
        {
            return orderDB.List(1, 0, "", 0, customerID).ToList();
        }
        ...
        /// <returns></returns>
        public static List<Order> ListOrders(int page, int pageSize, int status, string searchValue, int customerID, out int rowCount)
        {
            rowCount = orderDB.Count(status, searchValue, customerID);
            return orderDB.List(page, pageSize, searchValue, status, customerID).ToList();
        }
